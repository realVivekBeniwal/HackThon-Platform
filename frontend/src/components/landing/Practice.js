    ...
    <div className="max-w-6xl mx-auto py-12 px-4 space-y-20">
      ...
      <motion.div initial={{ opacity: 0, y: -40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="flex flex-col md:flex-row items-center gap-8 mb-12">
        <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ duration: 0.7, delay: 0.2 }} className="bg-purple-100 rounded-full p-8 shadow-lg mb-4 md:mb-0">
          <CodeBracketIcon className="h-16 w-16 text-purple-600 animate-bounce" />
        </motion.div>
        <div>
          <h1 className="text-4xl font-bold text-purple-900 mb-2">Practice & Prepare</h1>
          <p className="text-lg text-purple-700 mb-4">Sharpen your coding, interview, and problem-solving skills with real challenges and expert mentors.</p>
          <motion.button whileHover={{ scale: 1.08 }} className="px-6 py-3 bg-purple-600 text-white rounded-lg font-semibold shadow hover:bg-purple-700 transition">Start Practicing</motion.button>
        </div>
      </motion.div>
      {/* Practice Tracks */}
      <div>
        <h2 className="text-2xl font-bold text-purple-900 mb-6">Practice Tracks</h2>
          ...
          {practiceTracks.map((track, i) => {
            ...
            return (
              <motion.div key={track.name} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} whileHover={{ scale: 1.04 }} className={`glassmorphism bg-gradient-to-br ${track.color} rounded-xl shadow-xl p-6 flex items-center gap-6`}>
                <Icon className="h-12 w-12 text-purple-600 animate-bounce" />
                <div className="flex-1">
                  <h3 className="font-semibold text-purple-900 mb-1">{track.name}</h3>
                  <div className="w-full bg-purple-200 rounded-full h-3 mb-2">
                    <div className="bg-purple-600 h-3 rounded-full" style={{ width: `${track.progress}%` }}></div>
                  </div>
                  <span className="text-purple-700 text-sm">{track.progress}% completed</span>
                </div>
              </motion.div>
            );
          })}
      ...
      {/* Featured Challenges */}
      <div>
        <h2 className="text-2xl font-bold text-purple-900 mb-6">Featured Challenges</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {featuredChallenges.map((challenge, i) => {
            const Icon = challenge.icon;
            return (
              <motion.div key={challenge.title} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }} whileHover={{ scale: 1.04 }} className="bg-white rounded-xl shadow-lg p-6 flex items-center gap-6">
                <Icon className="h-10 w-10 text-purple-600 animate-bounce" />
                <div className="flex-1">
                  <h3 className="font-semibold text-purple-900 mb-1">{challenge.title}</h3>
                  <p className="text-purple-700 text-sm mb-1">{challenge.desc}</p>
                  <span className="text-xs text-purple-500">Mentor: {challenge.mentor}</span>
                </div>
                <motion.button whileHover={{ scale: 1.08 }} className="px-4 py-2 bg-purple-600 text-white rounded-lg font-semibold shadow hover:bg-purple-700 transition flex items-center">Join <ArrowRightIcon className="h-4 w-4 ml-1" /></motion.button>
              </motion.div>
            );
          })}
        </div>
      ...
      {/* Mock Interviews */}
      <div>
        <h2 className="text-2xl font-bold text-purple-900 mb-6">Mock Interviews</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {mockInterviews.map((mock, i) => (
            <motion.div key={mock.title} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }} whileHover={{ scale: 1.04 }} className="bg-purple-50 rounded-xl shadow-lg p-6 flex flex-col items-center">
              <ClipboardDocumentCheckIcon className="h-10 w-10 text-purple-600 mb-2 animate-pulse" />
              <h3 className="font-semibold text-purple-900 mb-1">{mock.title}</h3>
              <span className="text-purple-700 text-sm mb-1">Mentor: {mock.mentor}</span>
              <span className="text-xs text-purple-500 mb-2">{new Date(mock.date).toLocaleDateString()}</span>
      ...
      {/* Mentor Spotlights */}
      <div>
        <h2 className="text-2xl font-bold text-purple-900 mb-6">Mentor Spotlights</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {mentors.map((mentor, i) => (
            <motion.div key={mentor.name} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }} whileHover={{ scale: 1.04 }} className="bg-white rounded-xl shadow-lg p-6 flex flex-col items-center">
              ...
              <h3 className="font-semibold text-purple-900 mb-1">{mentor.name}</h3>
              <span className="text-purple-700 text-sm mb-2">{mentor.bio}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <AnimatePresence>
            {testimonials.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} transition={{ delay: i * 0.12 }} className="bg-purple-50 rounded-xl shadow-lg p-6 flex flex-col items-center">
                <img src={t.img} alt={t.name} className="h-14 w-14 rounded-full mb-2" />
                <p className="text-purple-800 mb-2 text-center">"{t.text}"</p>
                <div className="flex mb-1">
                  {[...Array(t.rating)].map((_, idx) => <StarIcon key={idx} className="h-5 w-5 text-yellow-400" />)}
                </div>
                <span className="font-semibold text-purple-900">{t.name}</span>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
